...
resource "azurerm_monitor_data_collection_endpoint" "dce_prometheus" {
  name                = "MSProm-${azurerm_kubernetes_cluster.aks_01.name}"
  resource_group_name = data.azurerm_resource_group.rg_01.name
  location            = data.azurerm_resource_group.rg_01.location
  kind                = "Linux"
}
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  name                = "NodeRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.aks_01.name}"
  location            = data.azurerm_resource_group.rg_01.location
  resource_group_name = data.azurerm_resource_group.rg_01.name
  cluster_name        = azurerm_kubernetes_cluster.aks_01.name
  description         = "Node Recording Rules Rule Group"
  rule_group_enabled  = true
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.amw.id, azurerm_kubernetes_cluster.aks_01.id]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "kubernetes_recording_rules_rule_group" {
  name                = "KubernetesRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.aks_01.name}"
  location            = data.azurerm_resource_group.rg_01.location
  resource_group_name = data.azurerm_resource_group.rg_01.name
  cluster_name        = azurerm_kubernetes_cluster.aks_01.name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_and_kubernetes_recording_rules_rule_group_win" {
  name                = "NodeAndKubernetesRecordingRulesRuleGroup-Win-${azurerm_kubernetes_cluster.aks_01.name}"
  location            = data.azurerm_resource_group.rg_01.location
  resource_group_name = data.azurerm_resource_group.rg_01.name
  cluster_name        = azurerm_kubernetes_cluster.aks_01.name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group_win" {
  name                = "NodeRecordingRulesRuleGroup-Win-${azurerm_kubernetes_cluster.aks_01.name}"
  location            = data.azurerm_resource_group.rg_01.location
  resource_group_name = data.azurerm_resource_group.rg_01.name
  cluster_name        = azurerm_kubernetes_cluster.aks_01.name
  description         = "Node and Kubernetes Recording Rules Rule Group for Windows Nodes"
